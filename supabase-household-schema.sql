...
as $$
  select exists (
    select 1 from public.corvus_household_members
...
begin
  ...
  if exists(select 1 from public.corvus_household_members where user_id=auth.uid()) then
    raise exception 'This account already belongs to a household';
  end if;
  insert into public.corvus_households(name,owner_id)
  values(coalesce(nullif(trim(household_name),''),'My Household'),auth.uid())
  returning id into new_id;
  insert into public.corvus_household_members(household_id,user_id,role)
  values(new_id,auth.uid(),'owner');
  insert into public.corvus_household_state(household_id,payload,updated_by)
...
begin
  select household_id into hid from public.corvus_household_members
  ...
  insert into public.corvus_household_invites(household_id,code,created_by)
...
begin
  ...
  select * into inv from public.corvus_household_invites
...
create policy "Members can read households" on public.corvus_households for select to authenticated
using (public.corvus_is_household_member(id));

drop policy if exists "Members can read household members" on public.corvus_household_members;
create policy "Members can read household members" on public.corvus_household_members for select to authenticated
using (public.corvus_is_household_member(household_id));

drop policy if exists "Owners can manage invites" on public.corvus_household_invites;
create policy "Owners can manage invites" on public.corvus_household_invites for select to authenticated
using (exists(select 1 from public.corvus_household_members m where m.household_id=corvus_household_invites.household_id and m.user_id=auth.uid() and m.role='owner'));

drop policy if exists "Members can read household state" on public.corvus_household_state;
create policy "Members can read household state" on public.corvus_household_state for select to authenticated
using (public.corvus_is_household_member(household_id));

drop policy if exists "Members can insert household state" on public.corvus_household_state;
create policy "Members can insert household state" on public.corvus_household_state for insert to authenticated
with check (public.corvus_is_household_member(household_id));

drop policy if exists "Members can update household state" on public.corvus_household_state;
create policy "Members can update household state" on public.corvus_household_state for update to authenticated
using (public.corvus_is_household_member(household_id))
with check (public.corvus_is_household_member(household_id));
...
grant execute on function public.corvus_create_household(text), public.corvus_create_invite(), public.corvus_join_household(text), public.corvus_is_household_member(uuid) to authenticated;